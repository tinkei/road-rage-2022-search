...
def setupLogging(filename):
    ...
    log.info('Logger created (Custom).')
...
class MotionPlanner:
    ...
    def __init__(self, scenario, planningProblem, automata):
        ...
        self.log = setupLogging('output_gbfs_only_time')
    ...
    def calc_heuristic_cost(self, path: List[State], curPos: State):
        ...
        if hasattr(self.planningProblem.goal.state_list[0], 'time_step'):
            ...
            if path[-1].time_step > self.planningProblem.goal.state_list[0].time_step.end:
            ...
            elif path[-1].time_step <= time_step_goal:

                dist_time = 1 - (path[-1].time_step / self.planningProblem.goal.state_list[0].time_step.end)
                ...
                dist_time = 1 - (dist_time_min / self.planningProblem.goal.state_list[0].time_step.end)

        else:

            dist_time = 1

        time_cost = weight_time * dist_time
        ...
        try:
            ...
            if final_lanelet_id is not None and len(final_lanelet_id) > 0:
                ...
                if is_goal_lane:
                    weight_obstacle /= 2
                if num_obstacles > 0:
                    weight_obstacle *= (num_obstacles + 1)

                # Calculate cost from deviating from lanelet center.
                if lanelet_distance is not None:
                    lanetlet_cost = weight_lanelet * lanelet_distance / (num_obstacles + 1) ** 2

                dist_to_obstacle = self.dist_to_closest_obstacle(final_lanelet_id[0], path[-1].position, path[-1].time_step)
                obstacle_cost = weight_obstacle * (1 / dist_to_obstacle)
                ...
                if start_lanelet_id is not None and len(start_lanelet_id) > 0:
                    if final_lanelet_id[0] != start_lanelet_id[0]:
                        ...
                        obstacle_cost -= weight_obstacle * (1 / dist_to_obstacle_s)
                ...
                if len(final_lanelet_id) > 1:
                    for k in range(1, len(final_lanelet_id)):
                        ...
                        obstacle_cost_periphery += weight_obstacle_periphery * (1 / dist_to_obstacle_periphery)
                ...
                if len(start_lanelet_id) > 1:
                    for k in range(1, len(start_lanelet_id)):
                        ...
                        obstacle_cost_periphery -= weight_obstacle_periphery * (1 / dist_to_obstacle_periphery_s)
        ...
        #     velo.append(path[k].velocity)
        #     velo_abs.append(1/abs(path[k].velocity+0.1))
        # velocity_cost = weight_velocity * sum(velo_abs) / (len(path))
    ...
    def search_alg_astar(self, list_successors_start_state: List[MotionPrimitive], max_tree_depth: int, dict_status = None) -> (List[State], List[MotionPrimitive]):
        """
        Returns the path and the used primitives

        :param list_successors_start_state: All possible primitives for the start state
        ...
        # Here an A* algorithm is demonstrated. #
        ...
        # inputs for PriorityQueue: (cost_g,                            : cumulative cost of the path so far
        #                            path,                              : current path
        ...
        self.priority_queue.put((cost_g,                              # : initial cost is 0 
                                ...
                                list_primitives_copied),              # : no primitives add to the list yet
                                priority)                             # : initial priority is 0

        # while the queue is not empty
        while not self.priority_queue.empty():
            # get the item with lowest cost from the queue
            # elements represented by _ are not used in the following

            (cost_g, path, list_successors_primitive_last, tree_depth, _, list_primitives) = self.priority_queue.get()
            list_primitives_copied = copy.copy(list_primitives)

            status = {'cost_current': cost_g, 
            ...
            for i in range(len(list_successors_primitive_last)):
                ...
                if path_primitive_translated is None: # https://commonroad.in.tum.de/forum/t/problems-about-the-original-motion-planner-a-star/115
                    continue
                ...
                # in A*, the final cost (f) = heuristic cost (h) + cumulative cost (g)

                cost_g_increment = calc_travelled_distance(path_primitive_translated)
                cost_g = cost_g + cost_g_increment
                cost_h = self.calc_heuristic_cost(path_primitive_translated, path[-1])
